...
1. **`op run` phase** — envlock re-invokes itself wrapped inside `op run --environment <id>`. The 1Password CLI resolves your secrets and injects `DOTENV_PRIVATE_KEY_<ENV>` (e.g. `DOTENV_PRIVATE_KEY_PRODUCTION`) into the child process environment, then hands control back.
